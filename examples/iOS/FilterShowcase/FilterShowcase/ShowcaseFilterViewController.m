...
#import "FGFilterMatte.h"
#import "FGFilter70s.h"
#import "FGFilterFilm1.h"
#import "FGFilterBlackAndWhiteRetro.h"
#import "FGFilterBlackAndWhiteReg.h"
...
#import "FGFilterMoody.h"
#import "FGFilterWarmth.h"
#import "FGFilterSharp.h"
#import "FGFilterFilm1.h"
#import "FGFilterFilm2.h"

@interface ShowcaseFilterViewController () <UIGestureRecognizerDelegate, ZDStickerViewDelegate>
@property (nonatomic, strong) GPUImagePicture* imageSrc;
@property (nonatomic, weak) UIImageView* imageView;
@property (nonatomic, weak) UIImageView* overlayImageView;
@property (nonatomic, weak) ZDStickerView* overlayStickerView;

//@property (nonatomic, weak) GPUImageView* overlayGpuImageView;
@property (nonatomic, strong) GPUImagePicture* overlayImageSrc;
@property (nonatomic, strong) GPUImageTransformFilter* overlayRotationFilter;

@property (nonatomic) CGFloat minOverlayScale;
@property (nonatomic) CGFloat maxOverlayScale;
@property (nonatomic) CGSize origOverlaySize;

@property (nonatomic) CGFloat currentRotation;
@property (nonatomic) CGFloat lastRotation;

@property (nonatomic) CGPoint gradientOrigin;
@property (nonatomic) CGFloat gradientScale;
...
{
    _image = image;
    _imageSrc = [[GPUImagePicture alloc] initWithImage:image];
}
...
- (void)viewDidLoad
{
    ...
    UIImageView* backupIv = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height)];
    backupIv.contentMode = UIViewContentModeScaleAspectFit;
    backupIv.backgroundColor = [UIColor clearColor];
    [self.view addSubview:backupIv];
    _imageView = backupIv;
...
{
    switch (filterType) {
        case GPUIMAGE_GRADIENT_RADIAL:
        case GPUIMAGE_GRADIENT:
        {
            GPUImagePicture* picture = [[GPUImagePicture alloc] initWithImage:self.image];
            [picture addTarget:(GPUImageView*)self.view];
        ...
        default:
        {
            ...
            // This is a documented bug, which doesnt seem to have a solution
            // http://stackoverflow.com/questions/23803436/gpuimage-imagefromcurrentframebuffer-returning-nil-sometimes-for-gpuimagelookupf
            // Lookup filters only work right when you create them, so re-create any of them
            if (newImage == nil && [filter isKindOfClass:[FGLookupFilter class]])
            {
                FGLookupFilter* lookupFilter;
                switch (filterType) {
                    case GPUIMAGE_MATTE:
                    {
                        lookupFilter = [[FGFilterMatte alloc] init];
                    }; break;
                    case GPUIMAGE_SHARP:
                    {
                        lookupFilter = [[FGFilterSharp alloc] init];
                    }; break;
                    case GPUIMAGE_MOODY:
                    {
                        lookupFilter = [[FGFilterMoody alloc] init];
                    }; break;
                    case GPUIMAGE_FILM1:
                    {
                        lookupFilter = [[FGFilterFilm1 alloc] init];
                    }; break;
                    case GPUIMAGE_70S:
                    {
                        lookupFilter = [[FGFilter70s alloc] init];
                    }; break;
                    case GPUIMAGE_FILM2:
                    {
                        lookupFilter = [[FGFilterFilm2 alloc] init];
                    }; break;
                    case GPUIMAGE_BAWRETRO:
                    {
                        lookupFilter = [[FGFilterBlackAndWhiteRetro alloc] init];
                    }; break;
                    case GPUIMAGE_BAWREG:
                    {
                        lookupFilter = [[FGFilterBlackAndWhiteReg alloc] init];
                    }; break;
                }
                lookupFilter.intensity = ((FGLookupFilter*)filter).intensity;
                newImage = [lookupFilter imageByFilteringImage:self.image];
            }
            else if (newImage == nil)
            {
                newImage = [filter imageByFilteringImage:self.image];
            }
            
            [self.delegate addFilterFromShowcaseFilterViewController:filter
...
- (void) enableInteractiveOverlay
{
//    self.overlayStickerView.hidden = NO;
//    UIPinchGestureRecognizer *pinchRecognizer = [[UIPinchGestureRecognizer alloc] initWithTarget:self action:@selector(zoomAction:)];
//    [pinchRecognizer setDelegate:self];
//    [self.view addGestureRecognizer:pinchRecognizer];
//    
//    UIRotationGestureRecognizer *rotationRecognizer = [[UIRotationGestureRecognizer alloc] initWithTarget:self action:@selector(rotationAction:)];
...
- (void) enableGradientGestures
{
    lastLocation = CGPointZero;
    
    self.gradientOrigin = CGPointMake(self.view.bounds.size.width * 0.5, self.view.bounds.size.height * 0.5);
    self.gradientScale  = 0.5f;
    self.gradientAngle  = 0.0f;
    
    // Orange Gradients
//    self.gradientStart = [UIColor colorWithRed:1.00 green:0.75 blue:0.32 alpha:1.00];
//    self.gradientEnd   = [UIColor colorWithRed:1.00 green:0.73 blue:0.00 alpha:0.00];
    
    // Purple to Light Blue Gradients
//    self.gradientStart = [[UIColor purpleColor] colorWithAlphaComponent:1.0];
//    self.gradientEnd   = [[UIColor blueColor] colorWithAlphaComponent:0.0];
    
    // Blue to OffWHite
//    self.gradientStart = [UIColor colorWithRed:0 green:180.0f/255.0f blue:1.0f alpha:1.0f];
//    self.gradientEnd   = [UIColor colorWithRed:250.0f/255.0f green:239.0f/255.0f blue:228.0f/255.0f alpha:0.0f];
    
    // Ice
//    self.gradientStart = [UIColor colorWithRed:(49.0f / 255.0f) green:(224.0f / 255.0f) blue:(230.0f / 255.0f) alpha:1.00f];
//    self.gradientEnd = [UIColor colorWithRed:(76.0f / 255.0f) green:(207.0f / 255.0f) blue:(222.0f / 255.0f) alpha:0.00f];
    
    // Vignette
    self.gradientStart = [UIColor colorWithRed:(0.0f / 255.0f) green:(0.0f / 255.0f) blue:(0.0f / 255.0f) alpha:0.00f];
    self.gradientEnd = [UIColor colorWithRed:(0.0f / 255.0f) green:(0.0f / 255.0f) blue:(0.0f / 255.0f) alpha:0.25f];
    ...
    GPUImagePicture* background = [[GPUImagePicture alloc] initWithImage:self.image];
    
}

- (void) enabledOverlayFeatures:(CGRect)gpuImageFrame
{
    if (CGRectEqualToRect(gpuImageFrame, CGRectZero))
    {
        gpuImageFrame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width * 0.5, [[UIScreen mainScreen] bounds].size.width);
    }
    
    self.maxOverlayScale = MIN([[UIScreen mainScreen] bounds].size.width / gpuImageFrame.size.width, [[UIScreen mainScreen] bounds].size.height / gpuImageFrame.size.height);
    self.minOverlayScale = MAX([[UIScreen mainScreen] bounds].size.width * 0.1f / gpuImageFrame.size.width, [[UIScreen mainScreen] bounds].size.height * 0.1f / gpuImageFrame.size.height);
    self.origOverlaySize = CGSizeMake(gpuImageFrame.size.width, gpuImageFrame.size.height);
    
    UIImageView* imageview = [[UIImageView alloc] initWithFrame:gpuImageFrame];
    imageview.contentMode = UIViewContentModeScaleAspectFit;
    imageview.backgroundColor = [UIColor clearColor];
    
    ZDStickerView *userResizableView1 = [[ZDStickerView alloc] initWithFrame:gpuImageFrame];
    ...
    userResizableView1.translucencySticker = YES;
    [userResizableView1 hideEditingHandles];
    [self.view addSubview:userResizableView1];
    
    self.overlayImageView = imageview;
    self.overlayStickerView = userResizableView1;
}

// For zoomAction
CGFloat lastScale;
CGFloat appendWidthAfterThreshold;
CGFloat appendHeightAfterThreshold;
- (void)zoomAction:(UIGestureRecognizer *)sender {
    ...
    if([gestureRecognizer state] == UIGestureRecognizerStateBegan)
    {
        ...
        appendWidthAfterThreshold  = 0;
        appendHeightAfterThreshold = 0;
    }
    
    if ([gestureRecognizer state] == UIGestureRecognizerStateBegan ||
        [gestureRecognizer state] == UIGestureRecognizerStateChanged)
    {
        CGFloat newScale = ([gestureRecognizer scale] - lastScale);
        
        self.gradientScale += newScale;
        
        self.gradientScale = MAX(0.05, MIN(2.0, self.gradientScale));
        
        [self refreshGradient];
        
//        NSLog(@"original scale %f", newScale);
//        newScale = MIN(newScale, self.maxOverlayScale);
//        newScale = MAX(newScale, self.minOverlayScale);
//        NSLog(@"new scale %f", newScale);
        
//        CGFloat appendWidth  = self.origOverlaySize.width  * newScale;
//        CGFloat appendHeight = self.origOverlaySize.height * newScale;
//        
//        appendWidthAfterThreshold  += appendWidth;
//        appendHeightAfterThreshold += appendHeight;
        
        lastScale = [gestureRecognizer scale];  // Store the previous scale factor for the next pinch gesture call
//        
//        CGRect newframe = CGRectMake(
//                                     self.overlayStickerView.frame.origin.x,
//                                     self.overlayStickerView.frame.origin.y,
//                                     self.overlayStickerView.frame.size.width  + appendWidthAfterThreshold,
//                                     self.overlayStickerView.frame.size.height + appendHeightAfterThreshold);
//        
//        NSLog(@"new frame %@", NSStringFromCGRect(newframe));
//        self.overlayStickerView.frame = newframe;
//        
//        // Redraws the image in the bigger gpuimageview
//        [self.overlayImageSrc processImage];
//        
//        appendWidthAfterThreshold  = 0;
//        appendHeightAfterThreshold = 0;
    }
...
//    {
////        CGRect newframe = CGRectMake(
////                                     self.overlayGpuImageView.frame.origin.x,
////                                     self.overlayGpuImageView.frame.origin.y,
////                                     self.overlayGpuImageView.frame.size.width  + appendWidthAfterThreshold,
////                                     self.overlayGpuImageView.frame.size.height + appendHeightAfterThreshold);
//        
//        CGRect newframe = CGRectMake(
//                                     self.overlayStickerView.frame.origin.x,
//                                     self.overlayStickerView.frame.origin.y,
...
- (void)panAction:(UIPanGestureRecognizer *)gesture
{
    ...
    {
        if (CGPointEqualToPoint(CGPointZero, lastLocation) == NO) {
            float changeX = [gesture locationInView:self.view].x - lastLocation.x;
            float changeY = [gesture locationInView:self.view].y - lastLocation.y;
            self.gradientOrigin = CGPointMake(
                  self.gradientOrigin.x + changeX,
                  self.gradientOrigin.y + changeY);
            [self refreshGradient];
        }
...
//        [self.overlayImageView setCenter:location];
        //[self.overlayGpuImageView setCenter:location];
    }
}
//
//// For rotationAction
//- (void)rotationAction:(UIRotationGestureRecognizer *)gesture {
//    self.currentRotation += gesture.rotation - self.lastRotation;
//    self.overlayImageView.transform = CGAffineTransformMakeRotation(gesture.rotation);
//    //[self.overlayImageSrc processImage];
//    self.lastRotation = gesture.rotation;
////    self.currentRotation += gesture.rotation - self.lastRotation;
////    [self.overlayRotationFilter setAffineTransform:CGAffineTransformMakeRotation(gesture.rotation)];
////    [self.overlayImageSrc processImage];
////    self.lastRotation = gesture.rotation;
//}
...
{
    if (filterType == GPUIMAGE_GRADIENT_RADIAL)
    {
        self.imageView.image = [FGGradientHelper createRadialGradientWithOrigin:self.gradientOrigin
                                                                        inFrame:self.view.bounds
                                                                        atScale:self.gradientScale
                                                                     startColor:self.gradientStart
                                                                       endColor:self.gradientEnd];
    }
    else
    {
        self.imageView.image = [FGGradientHelper createLinearGradientWithOrigin:self.gradientOrigin
                                                                        inFrame:self.view.bounds
                                                                        atScale:self.gradientScale
                                                                     angleInDeg:self.gradientAngle
                                                                     startColor:self.gradientStart
                                                                       endColor:self.gradientEnd];
    }
    
    NSLog(@"\n\norigin %@\n\nscale %f\n\nangle %f\n\n", NSStringFromCGPoint(self.gradientOrigin), self.gradientScale, self.gradientAngle);
    
    [self.view bringSubviewToFront:self.filterSettingsSlider];
...
//    videoCamera.outputImageOrientation = UIInterfaceOrientationPortrait;
    ...
    switch (filterType)
    {
        ...
        case GPUIMAGE_MATTE:
        {
            self.title = @"FG Matte (Lookup)";
            self.filterSettingsSlider.hidden = NO;
            self.filterSettingsSlider.value = 1.0f;
            
            filter = [[FGFilterMatte alloc] init];
        }; break;
        case GPUIMAGE_SHARP:
        {
            self.title = @"FG Sharp";
            ...
            filter = [[FGFilterSharp alloc] init];
        }; break;
        case GPUIMAGE_MOODY:
        {
            self.title = @"FG Moody";
            ...
            filter = [[FGFilterMoody alloc] init];
        }; break;
        case GPUIMAGE_FILM1:
        {
            self.title = @"FG Film 1";
            self.filterSettingsSlider.hidden = NO;
            self.filterSettingsSlider.value = 1.0f;
            
            filter = [[FGFilterFilm1 alloc] init];
        }; break;
        case GPUIMAGE_FILM2:
        {
            self.title = @"FG Film 2";
            self.filterSettingsSlider.hidden = NO;
            self.filterSettingsSlider.value = 1.0f;
            
            filter = [[FGFilterFilm2 alloc] init];
        }; break;
        case GPUIMAGE_FLARE:
        {
            self.title = @"FG Flare (Lookup & Gradient)";
            self.filterSettingsSlider.hidden = NO;
            self.filterSettingsSlider.value = 1.0f;
            
            FGFilterFlare* flareFilter = [[FGFilterFlare alloc] initWithInput:self.image];
            blendedImage = flareFilter.outputImage;
            filter = flareFilter;
        }; break;
        case GPUIMAGE_FLARE_ICE:
        {
            self.title = @"FG Flare Ice";
            self.filterSettingsSlider.hidden = NO;
            self.filterSettingsSlider.value = 1.0f;
            
            FGFilterFlareIce* flareFilter = [[FGFilterFlareIce alloc] initWithInput:self.image];
            blendedImage = flareFilter.outputImage;
            filter = flareFilter;
        }; break;
        case GPUIMAGE_WARMTH:
        {
            self.title = @"FG Warmth";
            self.filterSettingsSlider.hidden = NO;
            self.filterSettingsSlider.value = 1.0f;
            
            FGFilterWarmth* flareFilter = [[FGFilterWarmth alloc] initWithInput:self.image];
            blendedImage = flareFilter.outputImage;
            filter = flareFilter;
        }; break;
        case GPUIMAGE_NOISE_TEXTURE:
        {
            self.title = @"FG Kodak Noise Texture";
            self.filterSettingsSlider.hidden = NO;
            self.filterSettingsSlider.value = 1.0f;
            
            FGFilterNoise* noiseFilter = [[FGFilterNoise alloc] initWithInput:self.image];
            blendedImage = noiseFilter.outputImage;
            filter = noiseFilter;
        }; break;
        case GPUIMAGE_70S:
        {
            self.title = @"FG 70s (Lookup)";
            self.filterSettingsSlider.hidden = NO;
            self.filterSettingsSlider.value = 1.0f;
            
            filter = [[FGFilter70s alloc] init];
        }; break;
        case GPUIMAGE_BAWRETRO:
        {
            self.title = @"FG B&W Retro (Lookup)";
            self.filterSettingsSlider.hidden = NO;
            self.filterSettingsSlider.value = 1.0f;
            
            filter = [[FGFilterBlackAndWhiteRetro alloc] init];
        }; break;
        case GPUIMAGE_BAWREG:
        {
            self.title = @"FG B&W Reg (Lookup)";
            self.filterSettingsSlider.hidden = NO;
            self.filterSettingsSlider.value = 1.0f;
            
            filter = [[FGFilterBlackAndWhiteReg alloc] init];
        }; break;
        case GPUIMAGE_WASTED:
        {
            self.title = @"GTA Wasted";
            self.filterSettingsSlider.hidden = YES;
            needsSecondImage = YES;
            
            FGFilterWasted* wastedFilter = [[FGFilterWasted alloc] initWithInput:self.image];
            blendedImage = wastedFilter.outputImage;
            filter = wastedFilter;
        }; break;
...
//            needsSecondImage = YES;
//            
//            FGSvgOverlayFilter* svgFilter = [[FGSvgOverlayFilter alloc] initWithInput:self.image];
//            blendedImage = svgFilter.outputImage;
//            filter = svgFilter;
//        }; break;
            
        case GPUIMAGE_INTERACTIVE_OVERLAY:
        {
            self.title = @"FG Interactic Overlay";
            self.filterSettingsSlider.hidden = YES;
            needsSecondImage = YES;
            
            CGFloat aspectRatio = [FGFilterOverlayLogo overlayImageAspectRatio];
            CGFloat overlayWidth = self.view.bounds.size.width  * 0.5f;
            CGFloat overlayHeight = overlayWidth * aspectRatio;
            CGFloat overlayX = self.view.bounds.size.width - overlayWidth;
            CGFloat overlayY = [[UIScreen mainScreen] bounds].size.height - overlayHeight;
            CGRect gpuImageFrame = CGRectMake(overlayX, overlayY, overlayWidth, overlayHeight);
            
            [self enabledOverlayFeatures:gpuImageFrame];
            
            FGFilterOverlayLogo* overylayFilter = [[FGFilterOverlayLogo alloc] initWithInput:self.image];
//            blendedImage = overylayFilter.outputImage;
            filter = overylayFilter;
            
            self.imageView.image = self.image;
            self.overlayStickerView.hidden = NO;
            
            UIImage* overlayImageOrig = [overylayFilter overlayImageWithBaseSize:CGSizeMake(blendedImage.size.width, blendedImage.size.height)];
            
            self.overlayImageView.image = overlayImageOrig;
            
//
//            self.overlayRotationFilter = [[GPUImageTransformFilter alloc] init];
//            [self.overlayRotationFilter setAffineTransform:CGAffineTransformMakeRotation(0.0)];
//            
//            self.overlayImageSrc = [[GPUImagePicture alloc] initWithImage:overlayImageOrig];
//            [self.overlayImageSrc addTarget:self.overlayRotationFilter];
//            [self.overlayRotationFilter addTarget:self.overlayGpuImageView];
//            [self.overlayImageSrc processImage];
            
            [self enableInteractiveOverlay];
...
//            [videoCamera addTarget:cropFilter];
        ...
        if (blendedImage != nil)
        {
            self.imageView.image = blendedImage;
            [self.view bringSubviewToFront:self.filterSettingsSlider];
        }
        else
        {
            [self.imageSrc addTarget:filter];
            [filter addTarget:(GPUImageView*)self.view];
            [self.imageSrc processImage];
        }
        
        // fix for iphone rendering
        [self performSelector:@selector(refreshFilter) withObject:nil afterDelay:0.1];
    }
}

- (void) refreshFilter
{
    [self updateFilterFromSlider:self.filterSettingsSlider];
}
...
//        case GPUIMAGE_BILATERAL: [(GPUImageBilateralFilter *)filter setBlurSize:[(UISlider*)sender value]]; break;
        ...
        case GPUIMAGE_GRADIENT:
        {
            self.gradientAngle = 180 * [(UISlider*)sender value];
            [self refreshGradient];
        }; break;
        ...
        case GPUIMAGE_MATTE:
        {
            ((FGFilterMatte*)filter).intensity = [(UISlider *)sender value];
        }; break;
        case GPUIMAGE_SHARP:
        {
            ((FGFilterSharp*)filter).intensity = [(UISlider *)sender value];
        }; break;
        case GPUIMAGE_MOODY:
        {
            ((FGFilterMoody*)filter).intensity = [(UISlider *)sender value];
        }; break;
        case GPUIMAGE_FILM1:
        {
            ((FGFilterFilm1*)filter).intensity = [(UISlider *)sender value];
        }; break;
        case GPUIMAGE_FILM2:
        {
            ((FGFilterFilm2*)filter).intensity = [(UISlider *)sender value];
        }; break;
        case GPUIMAGE_FLARE:
        {
            ((FGFilterFlare*)filter).intensity = [(UISlider *)sender value];
            outputImage = ((FGFilterFlare*)filter).outputImage;
        } break;
        case GPUIMAGE_FLARE_ICE:
        {
            ((FGFilterFlareIce*)filter).intensity = [(UISlider *)sender value];
            outputImage = ((FGFilterFlareIce*)filter).outputImage;
        } break;
        case GPUIMAGE_WARMTH:
        {
            ((FGFilterWarmth*)filter).intensity = [(UISlider *)sender value];
            outputImage = ((FGFilterWarmth*)filter).outputImage;
        } break;
        case GPUIMAGE_NOISE_TEXTURE:
        {
            ((FGFilterNoise*)filter).intensity = [(UISlider *)sender value];
            outputImage = ((FGFilterNoise*)filter).outputImage;
        } break;
        case GPUIMAGE_70S:
            ((FGFilter70s*)filter).intensity = [(UISlider *)sender value];
            break;
        case GPUIMAGE_BAWRETRO:
            ((FGFilterBlackAndWhiteRetro*)filter).intensity = [(UISlider *)sender value];
            break;
        case GPUIMAGE_BAWREG:
            ((FGFilterBlackAndWhiteReg*)filter).intensity = [(UISlider *)sender value];
            break;
